...
(use-package pdf-view
  ...
  :init
  ;; Use the epdfinfo server built hermetically by nixpkgs (see nix-config's
  ;; modules/home/dev/emacs.nix) instead of letting pdf-tools try to compile
  ;; its own, which fails by mixing the system gcc with nixpkgs' poppler-glib.
...
* Claude Code
Claude Code integration for AI-assisted programming.
** Setup
#+begin_src emacs-lisp :tangle yes :results silent
(use-package! monet
  :config
...
(use-package! claude-code
  :after monet
  :config
  ;; IDE integration with Monet for file operations
...
** Smart Toggle
#+begin_src emacs-lisp :tangle yes :results silent
(defun my/claude-code-smart-toggle ()
  "Start Claude Code if not running, otherwise toggle its visibility."
  (interactive)
  (if (claude-code--find-claude-buffers-for-directory (claude-code--directory))
      (claude-code-toggle)
    (claude-code)))

(defun my/claude-code-here ()
  "Open Claude Code in a regular window, not a popup."
  (interactive)
  (let* ((dir (claude-code--directory))
         (existing-buf (car (claude-code--find-claude-buffers-for-directory dir))))
    (if existing-buf
        (switch-to-buffer existing-buf)
      ;; Start claude-code, then move the buffer to a regular window
      (claude-code)
      (when-let ((buf (car (claude-code--find-claude-buffers-for-directory dir))))
        (+popup/close)
...
(map!
 :leader
 (:prefix-map ("v" . "claude")
  :desc "Send region to Claude" "c" #'claude-code-send-region
  :desc "Continue conversation" "r" #'claude-code-continue
  :desc "Resume session" "s" #'claude-code-resume
  :desc "Fix error at point" "e" #'claude-code-fix-error-at-point
  :desc "Toggle Claude Code" "v" #'my/claude-code-smart-toggle
  :desc "Claude Code here" "V" #'my/claude-code-here
